class Solver {
    ...
    if (this.move != arr[this.row - 1][this.col - 1]) {
      ...
      if (this.neighbours.length != 0) {
        ...
        if (this.move != arr[this.row - 1][this.col - 1]) {
          fill(0, 255, 0);
          rect(
            this.move.x * this.cell_size,
            this.move.y * this.cell_size,
            this.cell_size,
            this.cell_size
          );
        }
  ...
  breadth_first_search() {
    if (this.move != arr[this.row - 1][this.col - 1]) {
      ...
      if (this.backtrack.previous != arr[0][0]) {
        ...
        fill(0, 255, 0);
        rect(
          this.backtrack.x * this.cell_size,
          this.backtrack.y * this.cell_size,
          this.cell_size,
          this.cell_size
        );
      } else {
  ...
  display_path() {
    ...
    for (let i = 0; i < this.openset.length; i++) {
      rect(
        this.openset[i].x * this.cell_size,
        this.openset[i].y * this.cell_size,
        this.cell_size,
        this.cell_size
      );
    }
    ...
    for (let i = 0; i < this.closedset.length; i++) {
      rect(
        this.closedset[i].x * this.cell_size,
        this.closedset[i].y * this.cell_size,
        this.cell_size,
        this.cell_size
      );
    }
    ...
    for (let i = 0; i < this.path.length; i++) {
      rect(
        this.path[i].x * this.cell_size,
        this.path[i].y * this.cell_size,
        this.cell_size,
        this.cell_size
      );
    }
